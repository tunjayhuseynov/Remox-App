...
const Li = ({ children, onClick }) => <li onClick={onClick} className="text-left border px-3 py-2 bg-white hover:bg-gray-200 cursor-pointer">{children}</li>

const Viewer = (name, address) => <div className="flex flex-col">
...
const Dropdown = ({ selected, list, price=false, onSelect }) => {
        ...
        <div className="relative">
            <div onClick={() => list?.length > 0 ? setOpen(!isOpen) : null} className={`flex justify-between items-center border rounded-xl ${isOpen && 'rounded-b-none'} py-2 px-3 cursor-pointer`}>
                ...
                {list && list.length > 0 && <div>
                    <IoIosArrowDown className='transition' style={isOpen ? { transform: "rotate(180deg)" } : null} />
                </div>}
            </div>
            {isOpen && <div className="absolute left-0 bottom-0 translate-y-full z-10 w-full overflow-hidden">
                <ul>
                    {list?.filter(!price?(w => w?.address !== selected?.address):(w=>w?.name !== selected?.name))?.map(w => <Li key={generate()} onClick={()=>{onSelect(w); setOpen(false)}}>
